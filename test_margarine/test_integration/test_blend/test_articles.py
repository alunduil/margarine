...
@unittest.skipUnless(test_helpers.is_vagrant_up('datastore'), 'requires datastore vagrant box')
class BlendArticleReadWithDatastoreTest(BaseBlendTest):
    ...
    def setUp(self):
        ...
        _ = os.path.join([ os.getcwd(), 'conf', 'margarine.ini' ])

        sys.argv.extend([ '-c', _ ])

        self.addCleanup(functools.partial(sys.argv.remove, '-c'))
        self.addCleanup(functools.partial(sys.argv.remove, _))
    ...
    def test_article_read_get_submitted_incomplete(self):
        ...
        for article in self.articles['all']:
            ...
            self.add_fixture_to_datastore(article['bson'])
